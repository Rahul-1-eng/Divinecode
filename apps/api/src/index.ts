...
type ContestProblem = { id: string; title: string; platform: string; url: string; difficulty?: string; rating?: number; tags: string[]; stdin?: string; expectedOutput?: string; sourceCode?: string };
...
function estimateRating(index: string, supplied?: number) {
  if (supplied) return supplied;
  const first = (index || 'A').toUpperCase()[0];
  return { A: 800, B: 1000, C: 1300, D: 1600, E: 1900, F: 2200 }[first as 'A'] || 1200;
}
function lookupPlatformProblem(platform: string, code: string) {
  const p = platform.toLowerCase();
  if (p.includes('codeforces')) {
    const parsed = parseCodeforcesCode(code);
    const rating = estimateRating(parsed.problemIndex);
    return { platform: 'Codeforces', contestCode: parsed.contestCode, problemIndex: parsed.problemIndex, title: `Codeforces ${parsed.contestCode}${parsed.problemIndex}`, url: buildProblemUrl('Codeforces', parsed.contestCode, parsed.problemIndex, ''), rating, difficulty: String(rating), tags: ['implementation'] };
  }
  if (p.includes('leetcode')) return { platform: 'LeetCode', contestCode: code.trim(), problemIndex: '', title: code.trim().split('-').map((x) => x[0]?.toUpperCase() + x.slice(1)).join(' '), url: buildProblemUrl('LeetCode', code.trim(), '', ''), rating: 1200, difficulty: 'Medium', tags: ['leetcode'] };
  if (p.includes('atcoder')) return { platform: 'AtCoder', contestCode: code.trim().split('_')[0], problemIndex: code.trim().split('_')[1] || 'A', title: `AtCoder ${code}`, url: buildProblemUrl('AtCoder', code.trim().split('_')[0], code.trim().split('_')[1] || 'A', ''), rating: 1000, difficulty: 'Practice', tags: ['atcoder'] };
  if (p.includes('codechef')) return { platform: 'CodeChef', contestCode: code.trim(), problemIndex: '', title: `CodeChef ${code}`, url: buildProblemUrl('CodeChef', code.trim(), '', ''), rating: 1000, difficulty: 'Practice', tags: ['codechef'] };
  return { platform, contestCode: code, problemIndex: '', title: `${platform} ${code}`, url: '', rating: 1000, difficulty: 'Practice', tags: [platform.toLowerCase()] };
}
function generateMcqsFromProblems(contestProblems: ContestProblem[]): McqQuestion[] {
  const source = contestProblems.length ? contestProblems : problems.map((p) => ({ id: String(p.id), title: p.title, platform: 'DivineCode', url: `/problems/${p.id}`, difficulty: String(p.difficulty), rating: p.difficulty, tags: p.tags }));
  ...
  return source.flatMap((problem, index) => {
    ...
    return [
      ...
      { id: index * 3 + 3, question: `What rating label is attached to “${problem.title}”?`, options: [String(problem.rating || problem.difficulty || 'Practice'), ...difficulties.filter((d) => d !== problem.difficulty)].slice(0, 4), correctIndex: 0, concept: 'rating reading' }
    ];
...
app.post('/api/contests', (req, res) => {
  ...
  const safeProblems: ContestProblem[] = Array.isArray(contestProblems) ? contestProblems.map((p: any) => {
    const platform = String(p.platform || 'External'); const contestCode = String(p.contestCode || p.code || ''); const problemIndex = String(p.problemIndex || ''); const url = buildProblemUrl(platform, contestCode, problemIndex, String(p.url || ''));
    return { id: id('problem'), title: String(p.title || `${platform} ${contestCode}${problemIndex}`).trim(), platform, url, difficulty: p.difficulty ? String(p.difficulty) : problemIndex || undefined, rating: Number(p.rating || estimateRating(problemIndex)), tags: String(p.tags || platform).split(',').map((t: string) => t.trim()).filter(Boolean), stdin: String(p.stdin || ''), expectedOutput: String(p.expectedOutput || ''), sourceCode: `${contestCode}${problemIndex}` };
  ...
  rebuildStandings(contest); contests.set(contest.id, contest); persistContest(contest); res.status(201).json(contest);
});
app.get('/api/contests/:id', (req, res) => { const contest = contests.get(req.params.id); if (!contest) { res.status(404).json({ error: 'Contest not found' }); return; } rebuildStandings(contest); res.json(contest); });
app.get('/api/contests/:id/submissions', (req, res) => { res.json(submissions.filter((s) => s.contestId === req.params.id).slice(-100).reverse()); });
app.post('/api/contests/:id/members', (req, res) => { const contest = contests.get(req.params.id); if (!contest) { res.status(404).json({ error: 'Contest not found' }); return; } uniqueByName(Array.isArray(req.body.members) ? req.body.members : [req.body.name]).forEach((name) => { if (!contest.members.some((member) => member.name.toLowerCase() === name.toLowerCase())) contest.members.push({ id: id('member'), name, handle: slugHandle(name) }); }); rebuildStandings(contest); persistContest(contest); res.json(contest); });
app.post('/api/contests/:id/problems', (req, res) => { const contest = contests.get(req.params.id); if (!contest) { res.status(404).json({ error: 'Contest not found' }); return; } const platform = String(req.body.platform || 'External'); const problem: ContestProblem = { id: id('problem'), title: String(req.body.title || 'Untitled Problem'), platform, url: buildProblemUrl(platform, String(req.body.contestCode || ''), String(req.body.problemIndex || ''), String(req.body.url || '')), difficulty: req.body.difficulty ? String(req.body.difficulty) : undefined, rating: Number(req.body.rating || estimateRating(String(req.body.problemIndex || ''))), tags: String(req.body.tags || platform).split(',').map((t: string) => t.trim()).filter(Boolean), stdin: String(req.body.stdin || ''), expectedOutput: String(req.body.expectedOutput || ''), sourceCode: `${req.body.contestCode || ''}${req.body.problemIndex || ''}` }; if (!problem.url) { res.status(400).json({ error: 'Problem URL is required' }); return; } contest.problems.push(problem); contest.questions = generateMcqsFromProblems(contest.problems); rebuildStandings(contest); persistContest(contest); res.json(contest); });
app.post('/api/contests/:id/solve', (req, res) => { const contest = contests.get(req.params.id); if (!contest) { res.status(404).json({ error: 'Contest not found' }); return; } const { memberId, problemId } = req.body; const member = contest.members.find((m) => m.id === memberId); const problem = contest.problems.find((p) => p.id === problemId); if (!member || !problem) { res.status(400).json({ error: 'Invalid member or problem' }); return; } recordSolve(contest, memberId, problemId); res.json(contest); });
app.post('/api/contests/:id/unsolve', (req, res) => { const contest = contests.get(req.params.id); if (!contest) { res.status(404).json({ error: 'Contest not found' }); return; } const { memberId, problemId } = req.body; contest.solves = contest.solves.filter((s) => !(s.memberId === memberId && s.problemId === problemId)); rebuildStandings(contest); persistContest(contest); res.json(contest); });
...
async function runWithJudge0(params: { sourceCode: string; language: JudgeLanguage; stdin: string; expectedOutput: string }) {
  ...
  if (!params.expectedOutput) return { verdict: 'Submitted', message: 'Linked external problem. Open the original problem, submit there, then use DivineCode judge sync when configured.', stdout: '', stderr: '', compile_output: '' };
  if (!judgeUrl) return { verdict: 'Queued', message: 'Judge0 is not configured yet. Submission stored and visible in standings history.', stdout: '', stderr: '', compile_output: '' };
  ...
  const result = (await createResponse.json()) as Judge0Result; const statusDescription = result.status?.description || 'Unknown'; let verdict = statusDescription;
...
app.post('/api/submit', async (req, res) => {
  try {
    ...
    let stdin = ''; let expectedOutput = ''; let contest: Contest | undefined;
    if (contestId) { contest = contests.get(String(contestId)); if (!contest) { res.status(404).json({ verdict: 'Rejected', message: 'Contest not found' }); return; } const contestProblem = contest.problems.find((item) => item.id === String(problemId)); if (!contestProblem) { res.status(404).json({ verdict: 'Rejected', message: 'Contest problem not found' }); return; } stdin = contestProblem.stdin || ''; expectedOutput = contestProblem.expectedOutput || ''; }
    else { const problem = problems.find((item) => item.id === Number(problemId)); if (!problem) { res.status(404).json({ verdict: 'Rejected', message: 'Problem not found' }); return; } stdin = problem.stdin; expectedOutput = problem.expectedOutput; }
    const result = await runWithJudge0({ sourceCode: code, language: language as JudgeLanguage, stdin, expectedOutput });
    if (result.verdict === 'Accepted' && contest && memberId) recordSolve(contest, String(memberId), String(problemId));
    const saved = { id: id('sub'), userId, memberId, contestId, problemId, language, code, verdict: result.verdict, message: result.message, stdout: result.stdout, stderr: result.stderr, compileOutput: result.compile_output, time: result.time ? String(result.time) : undefined, memory: typeof result.memory === 'number' ? result.memory : undefined, createdAt: new Date().toISOString() };
    submissions.push(saved); void saveSubmissionDocument(saved).catch((error) => console.error('Could not save submission:', error instanceof Error ? error.message : error));
    res.json({ ...result, language, problemId, contestId: contest?.id, standings: contest?.standings || null });
  } catch (error) { res.status(500).json({ verdict: 'Server Error', message: error instanceof Error ? error.message : 'Unknown error' }); }
});
...
io.on('connection', (socket) => { socket.on('duel:join', (payload: { name?: string }) => { const player: Player = { id: socket.id, name: payload?.name || `Player-${socket.id.slice(0, 4)}`, score: 0 }; if (waitingPlayers.length === 0) { waitingPlayers.push(player); socket.emit('duel:waiting', { message: 'Waiting for another player...' }); return; } const opponent = waitingPlayers.shift()!; const roomId = `room-${Date.now()}`; const seedContest = [...contests.values()].at(-1); const room: DuelRoom = { id: roomId, players: [opponent, player], questionIndex: 0, questions: seedContest?.questions?.length ? seedContest.questions : generateMcqsFromProblems([]), finished: false }; rooms.set(roomId, room); socket.join(roomId); io.sockets.sockets.get(opponent.id)?.join(roomId); io.to(roomId).emit('duel:start', { roomId, players: room.players }); emitRoom(room); }); socket.on('duel:answer', (payload: { roomId: string; questionId: number; answerIndex: number }) => { const room = rooms.get(payload.roomId); if (!room || room.finished) return; const current = room.questions[room.questionIndex]; const player = room.players.find((item) => item.id === socket.id); if (!player || current.id !== payload.questionId) return; const correct = current.correctIndex === payload.answerIndex; player.score += correct ? 10 : -3; io.to(room.id).emit('duel:feedback', { playerId: socket.id, playerName: player.name, correct, correctIndex: current.correctIndex, concept: current.concept }); room.questionIndex += 1; if (room.questionIndex >= room.questions.length) room.finished = true; setTimeout(() => emitRoom(room), 700); }); socket.on('disconnect', () => { const waitingIndex = waitingPlayers.findIndex((p) => p.id === socket.id); if (waitingIndex >= 0) waitingPlayers.splice(waitingIndex, 1); }); });
