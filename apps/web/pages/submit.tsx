...
export default function SubmitPage() {
  ...
  useEffect(() => { if (!contestId) return; fetch(`${API_BASE_URL}/api/contests/${contestId}`).then((r) => r.json()).then((data) => { setContest(data); setProblem(data.problems?.find((p: any) => p.id === problemId)); }).catch(() => null); }, [contestId, problemId]);
  const isCodeforces = problem?.platform?.toLowerCase?.().includes('codeforces');
  async function submitCode() { if (!session?.user?.email) return alert('Sign in first'); if (!contest || !problem) return alert('Contest problem not loaded'); setSubmitting(true); setVerdict(null); const member = contest.members?.find((m: any) => m.name === session.user?.name || m.name === session.user?.email) || contest.members?.[0]; const res = await fetch(`${API_BASE_URL}/api/submit`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ code, language, contestId, problemId, userId: session.user.email, memberId: member?.id }) }); const data = await res.json(); setSubmitting(false); setVerdict(data); }
  ...
  return <main style={page}><nav style={nav}><a href={contestId ? `/contests/${contestId}` : '/contests'} style={brand}>← Back to contest</a><div style={userPill}>{session.user?.name || session.user?.email}</div></nav><section style={layout}><aside style={panel}><p style={eyebrow}>{isCodeforces ? 'External verified submission' : 'DivineCode local judge'}</p><h1>{problem?.title || 'Loading problem...'}</h1><p style={{ color: '#94a3b8' }}>{problem?.platform} · Rating {problem?.rating || problem?.difficulty || 'Practice'} · {(problem?.tags || []).join(', ')}</p>{isCodeforces && <div style={warning}><strong>Codeforces problem</strong><p>Submit your solution on Codeforces. DivineCode will not mark this Accepted from local code. Standings update only after Codeforces API sync finds verdict OK.</p></div>}{problem?.url && <a href={problem.url} target="_blank" rel="noreferrer" style={primaryLink}>{isCodeforces ? 'Open and Submit on Codeforces' : 'Open original problem'}</a>}<div style={{ marginTop: 22 }}><label>Language</label><select value={language} onChange={(e) => setLanguage(e.target.value)} style={input}><option value="cpp">C++</option><option value="java">Java</option><option value="python">Python</option><option value="javascript">JavaScript</option><option value="c">C</option></select></div><button onClick={submitCode} disabled={submitting} style={submitBtn}>{submitting ? 'Submitting...' : isCodeforces ? 'Store as Pending Verification' : 'Submit to DivineCode Judge'}</button>{verdict && <div style={verdictBox}><h2>{verdict.verdict}</h2><p>{verdict.message}</p><a href={`/contests/${contestId}`} style={primaryLink}>Back to live standings</a></div>}</aside><section style={editorPanel}><div style={editorTop}><strong>{isCodeforces ? 'Scratchpad only' : 'Code editor'}</strong><span>{isCodeforces ? 'Use Codeforces for official verdict' : 'Judge0-ready execution'}</span></div><textarea value={code} onChange={(e) => setCode(e.target.value)} spellCheck={false} style={editor} /></section></section></main>;
}
