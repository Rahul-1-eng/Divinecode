...
export default function ContestRoomPage() {
  ...
  async function syncCodeforces() { if (!id) return; setSyncing(true); const res = await fetch(`${API_BASE_URL}/api/contests/${id}/sync/codeforces`, { method: 'POST' }); const data = await res.json(); setSyncing(false); if (!res.ok) return alert(data.error || 'Sync failed'); await loadContest(); await loadSubmissions(); alert(`Synced ${data.synced?.length || 0} accepted submission(s).`); }
  ...
  return (
    <main style={page}>
      <section style={{ maxWidth: 1240, margin: '0 auto' }}>
        <nav style={nav}><a href="/" style={link}>← DivineCode</a><div style={userPill}>{session.user?.name || session.user?.email}</div></nav>
        <div style={hero}>
          <div><p style={eyebrow}>Verified Codeforces Gym-style mashup</p><h1 style={{ fontSize: 46, margin: 0 }}>{contest.title}</h1><p style={{ color: '#a8b3c7' }}>Local submits for Codeforces stay pending. Only Codeforces API Accepted or true Judge0 Accepted updates standings.</p></div>
          <div style={timerCard}><strong>{Math.floor(timeLeft / 60)}:{String(timeLeft % 60).padStart(2, '0')}</strong><span>remaining</span></div>
        </div>
        <div style={grid}>
          <section style={panel}>
            <h2>Owner controls</h2>
            <button onClick={syncCodeforces} disabled={syncing} style={primaryButton}>{syncing ? 'Syncing...' : 'Sync Codeforces'}</button>
            <button onClick={() => extendTime(15)} style={ghostButton}>+15 min</button>
            <button onClick={() => extendTime(30)} style={ghostButton}>+30 min</button>
            <h2>Participants</h2>
            {contest.members.map((m: any) => <p key={m.id} style={{ color: '#cbd5e1' }}>{m.name}<br/><span style={{ color: '#67e8f9' }}>CF: {m.codeforcesHandle || m.handle || 'missing'}</span></p>)}
            ...
            <div style={{ display: 'grid', gap: 12 }}>
              {contest.problems.map((p: any, index: number) => (
                <div key={p.id} style={problemRow}>
                  ...
                  <div><a href={p.url} target="_blank" rel="noreferrer" style={{ color: '#eef2ff', fontWeight: 900 }}>{p.title}</a><p style={{ margin: '6px 0 0', color: '#94a3b8' }}>{p.platform} · Rating {p.rating || p.difficulty || 'Practice'} · {(p.tags || []).join(', ')}</p></div>
                  <a href={`/submit?contestId=${contest.id}&problemId=${p.id}`} style={primaryLink}>{p.platform?.toLowerCase().includes('codeforces') ? 'Pending/Submit' : 'Submit'}</a>
                </div>
              ))}
            </div>
          </section>
